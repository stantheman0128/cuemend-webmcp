...
| `28255` | Testing instructions | Open the live URL in a WebMCP-enabled ChatGPT in-app browser or supported Chrome build. Confirm **WebMCP ready**, click **Copy judge prompt**, and send it to the browser agent. Ordinary-UI fallback: **r1 / 7 conflicts** → protect Jon's breath → **r2 / 8 conflicts + Stale** → replan revision 2, select plan 1, approve and apply → **r3 / 0 conflicts** → verify and build the evidence pack. | Native Chrome CDP run and ordinary-UI run are archived in `docs/verification/`. |
